...
@section('content')
    <div class="container mt-2">
        ...
        <div class="d-flex justify-content-between align-items-center">
            <h5 class="fw-bold my-primary-color">
                <i class="fas fa-file-invoice me-2"></i> {{ $order->proforma_number }}
            </h5>
            ...
            <div class="d-flex gap-2">
                ...
                @if ($order->status == 'draft' || $order->status == 'issued' || $order->status === 'sent')
                    <button class="btn btn-sm btn-primary update-proforma-btn" data-bs-toggle="modal" data-bs-target="#updateProformaModal" data-proforma='@json($order)' data-update-url="{{ route('proformas.update', $order->id) }}">
        ...
        <div class="mb-3">
            <small class="text-muted">
                ...
                <a class="text-decoration-none" href="{{ route('proformas.index') }}">Proforma Invoice</a>
                <i class="fa-solid fa-angle-right"></i>
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane fade show active" id="tab-details">
                <div class="card shadow-sm mb-3">
                    <div class="card-body">
                        ...
                        <hr>
                        <p><strong>Proforma Number:</strong> {{ $order->proforma_number }}</p>
                        <p><strong>Proforma Date:</strong> {{ $order->proforma_date }}</p>
                        <p><strong>Client Query From:</strong> {{ $order->client_query_from }}</p>
                        @if($order->quotation)
                            <p>
                        ...
                        @if($order->invoice)
                            <p>
                                ...
                                <a href="{{ route('invoices.show', $order->invoice->id) }}">
                                    <span class="badge bg-warning">{{ $order->invoice->invoice_number }}</span>
                                </a>
                            </p>
                        @endif
                        ...
                        <p><strong>Status:</strong>
                            <span class="badge 
                                ...
                                @elseif($order->status == 'sent') bg-warning
                                @elseif($order->status == 'issued') bg-warning
                                @elseif($order->status == 'converted') bg-info
                    ...
                    <div class="card-body">
                        ...
                        @if($order->items->count())
                            ...
                            <div class="d-flex justify-content-end mb-2">
                                @if($order->status === 'draft' || $order->status === 'issued' || $order->status === 'sent')
                                    <button type="button" data-proforma='@json($order)' class="btn btn-sm btn-primary edit-items-btn" id="openEditItemsModal" data-bs-toggle="modal" data-bs-target="#editItemsModal">
                            ...
                            <table class="table table-bordered" id="itemsTable" data-order-id="{{ $order->id }}">
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <th colspan="2">₹ {{ inr_format($order->subtotal) }}</th>
                                    </tr>
                                    @php
                                    ...
                                    @if($order->tax_type === 'cgst_sgst')
                                        <tr>
                                            <th colspan="10" class="text-end">CGST @if($commonTaxRate) ({{ number_format($commonTaxRate * $cgstRatio / $totalRatio, 2) + 0 }}%) @endif</th>
                                            <th colspan="2">₹ {{ inr_format($order->cgst_amount) }}</th>
                                        </tr>
                                        <tr>
                                            <th colspan="10" class="text-end">SGST @if($commonTaxRate) ({{ number_format($commonTaxRate * $sgstRatio / $totalRatio, 2) + 0 }}%) @endif</th>
                                            <th colspan="2">₹ {{ inr_format($order->sgst_amount) }}</th>
                                        </tr>
                                    @else
                                        <tr>
                                            <th colspan="10" class="text-end">IGST @if($commonTaxRate) ({{ number_format($commonTaxRate, 2) + 0 }}%) @endif</th>
                                            <th colspan="2">₹ {{ inr_format($order->igst_amount) }}</th>
                                        </tr>
                                    @endif
                                    <tr>
                                        <th colspan="10" class="text-end">Grand Total</th>
                                        <th colspan="2">₹ {{ inr_format($order->grand_total) }}</th>
                                    </tr>
                                </tfoot>
                            </table>
                        @else
            ...
            @php
                $canSendMail = in_array($order->status, ['sent', 'issued']);
            @endphp

            {{-- COMMUNICATION TAB --}}
            <div class="tab-pane fade" id="tab-communication">
                <div class="card shadow-sm mb-3
                    {{ !$canSendMail ? 'opacity-50' : '' }}">
                    
                    <div class="card-body"
                        style="{{ !$canSendMail ? 'pointer-events:none;' : '' }}">

                        {{-- SEND EMAIL --}}
                        <form action="{{ route('sales-orders.proformas.send-mail', $order->id) }}" method="POST">
                            @csrf
                            ...
                            <div class="row g-2">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input type="text"
                                        name="subject"
                                        class="form-control form-control-sm"
                                        value="Request For Proforma {{ $order->proforma_number }}"
                                        {{ !$canSendMail ? 'disabled' : '' }}>
                                </div>
                        ...
                        @if(!$canSendMail)
                            <div class="alert alert-warning mt-3 mb-0 small">
                                ...
                                Email can be sent only after the Proforma is <strong>Issued</strong> or already <strong>sent</strong>.
                            </div>
                        @endif
            ...
            <div class="tab-pane fade" id="tab-qr">
                <div class="card shadow-sm mb-3 text-center">
                    <div class="card-body">
                        ...
                        <p class="text-muted mt-2">QR encodes: <strong>{{ $order->proforma_number }}</strong></p>
        ...
        <div class="modal fade" id="editItemsModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    ...
                    <form id="editItemsForm" method="POST" action="{{ route('sales-orders.proformas.items.update', $order) }}">
                        @csrf
                        @method('PUT')

                        <div class="modal-header">
                            <h5 class="modal-title my-primary-color">Edit Proforma Items</h5>
        ...
        <div class="d-flex gap-2 mt-3 mb-4">
            @if($order->status === 'draft' || $order->status === 'sent')
                <form action="{{ route('sales-orders.proformas.accept', $order->id) }}" method="POST">
                    @csrf
                    <button type="submit"
                            class="btn btn-success"
                            onclick="return confirm('Accepted by customer?');">
                        <i class="fas fa-check-circle"></i> Accepted by Customer
                    </button>
                </form>
            @endif
           

            <a href="{{ route('sales-orders.proformas.print', $order->id) }}" class="btn btn-dark">
            ...
            <div class="d-flex gap-2">
                ...
                @if(($order->status === 'accepted') && !$order->invoice)
                        
                        <button class="btn btn-success" data-bs-target="#generateInvoiceModal" data-bs-toggle="modal">
                            <i class="fas fa-receipt"></i> Generate Tax Invoice
                        </button>
                @endif
    ...
    <div class="modal fade" id="updateProformaModal" tabindex="-1">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <form id="updateProformaForm" method="POST" action="{{ route('proformas.update', $order) }}">
                    @csrf
                    @method('PUT')

                    <div class="modal-header">
                        <h5 class="modal-title my-primary-color">Update Proforma</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>

                    <div class="modal-body">
                        <input type="hidden" id="update_proforma_id">

                        <div class="row g-3">

                            <div class="col-md-3">
                                <label class="form-label required-field">PI Date</label>
                                <input type="date" class="form-control" name="proforma_date" value="{{ $order->proforma_date }}" id="updateProformaDate">
                            </div>

                            <div class="col-md-3">
                                <label class="form-label required-field">PI No</label>
                                <input type="text" class="form-control" id="updateProformaNumber" value="{{ $order->proforma_number }}" disabled>
                                <input type="hidden" name="proforma_number" value="{{ $order->invoice_number }}" id="updateProformaNumberHidden">
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" value="{{ $order->client_query_from }}" name="client_query_from" class="form-control" required>
                            </div>

                            <div class="col-md-4">
                                <label class="form-label required-field">Tax Type</label>
                    ...
                    <div class="modal-footer">
                        ...
                        <button class="btn btn-success">Update PI</button>
                    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title">
                        Generate Invoice against this Proforma
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>

                <!-- Modal Body -->
                <form action="{{ route('proformas.convertToInvoice', $order->id) }}"
                    method="POST"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">
                                Client PO Reference<span class="text-danger">*</span>
                            </label>
                            <input type="text"
                                name="client_po_ref"
                                class="form-control"
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit"
                                class="btn btn-success"
                                onclick="return confirm('Are you sure you want to convert this PI to Tax Invoice?')">
                            <i class="fas fa-file-invoice"></i> Convert to Tax Invoice
                        </button>
                    </div>
...
@push('scripts')
    ...
    @vite('resources/js/pages/sales-order/proforma/show.js')
@endpush