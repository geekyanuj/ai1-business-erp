...
<body>
        ...
        <tr>
            ...
            <td width="65mm" valign="top">
                ...
                <table width="100%" cellpadding="3">
                    <tr>
                        <td width="25mm">PI No.</td>
                        <td width="45mm" align="right">
                            <strong>#{{ $so->proforma_number }}</strong>
                        </td>
                    </tr>
                    <tr>
                        <td width="25mm">Date</td>
                        <td width="40mm" align="right">{{ $so->proforma_date }}</td>
                    </tr>
                    <tr>
    ...
    <table class="items-table" cellpadding="3" cellspacing="0" width="190mm">
        <thead>
            <tr>
                <th style="width:7mm" class="text-center">#</th>
                <th style="width:55mm">Item & Description</th>
                <th style="width:14mm" class="text-center">HSN</th>
                <th style="width:10mm" class="text-right">Qty</th>
                <th style="width:18mm" class="text-right">Rate</th>
                <th style="width:11mm" class="text-right">Disc<br>(%)</th>
                <th style="width:11mm" class="text-right">Disc</th>
                <th style="width:20mm" class="text-right">Taxable</th>
        ...
        <tbody>
            @foreach($so->items as $i => $item)
                <tr>
                    <td style="width:7mm" class="text-center muted">{{ $i + 1 }}</td>
                    ...
                    <td style="width:55mm">
                        ...
                        @endif
                        @if($item->product->uom)
                            <span class="muted" style="font-size:4px;">UOM: {{ $item->product->uom }}</span>
                        @endif
                    </td>

                    <td style="width:14mm" class="text-center">{{ $item->product->hsn }}</td>
                    <td style="width:10mm" class="text-right">{{ $item->quantity }}</td>
                    <td style="width:18mm" class="text-right">₹{{ inr_format($item->unit_price) }}</td>
                    <td style="width:11mm" class="text-right">{{ number_format($item->discount_percent) }}%</td>
                    <td style="width:11mm" class="text-right">₹{{ inr_format($item->discount_amount ?? 0) }}</td>
                    <td style="width:20mm" class="text-right">₹{{ inr_format($item->taxable_amount) }}</td>
                    <td style="width:10mm" class="text-right">{{ number_format($item->tax_rate) }}%</td>
                    <td style="width:14mm" class="text-right">₹{{ inr_format($item->tax_amount) }}</td>
                    <td style="width:20mm" class="text-right bold">₹{{ inr_format($item->total_with_tax) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
                    ...
                    <tr>
                        ...
                        @else
                            <td style="font-size:8px;">• Prices are based on current specifications and
                                ...
                                • Proforma is valid for 15 days from the date of issue.</td><span><br></span>
                        @endif
    ...
    <table width="100%" cellpadding="4" cellspacing="0">
        <tr>
            ...
            <td width="110mm" valign="top">
                ...
                @else
                    ...
                    <div style="font-size:8px;"><span></span>
                        • This proforma is not a tax invoice and is subject to confirmation.<br><span></span>
            ...
            <td width="70mm" valign="bottom" align="center" style="text-align:center;">
                ...
                @if(($so->status === 'issued' || $so->status === 'sent') && $company->authorised_signature)
