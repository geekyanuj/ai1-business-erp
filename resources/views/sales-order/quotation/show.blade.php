...
@section('content')
    <div class="container mt-2">
        ...
        <div class="d-flex justify-content-between align-items-center">
            <h5 class="fw-bold my-primary-color">
                <i class="fas fa-file-invoice me-2"></i> {{ $order->quotation_number }}
            </h5>
            ...
            <div class="d-flex gap-2">
                ...
                @if ($order->status == 'draft' || $order->status === 'sent')
                    <button class="btn btn-sm btn-primary update-quotation-btn"
                        data-bs-toggle="modal"
                        data-bs-target="#updateQuotationModal"
                        data-quotation='@json($order)'
                        data-update-url="{{ route('quotations.update', $order->id) }}">
        ...
        <div class="mb-3">
            <small class="text-muted">
                ...
                <a class="text-decoration-none" href="{{ route('quotations.index') }}">Quotation</a>
                <i class="fa-solid fa-angle-right"></i> {{ $order->quotation_number }}
            </small>
        </div>
            ...
            <div class="tab-pane fade show active" id="tab-details">
                <div class="card shadow-sm mb-3">
                    <div class="card-body">
                        ...
                        <hr>
                        <p><strong>Quotation Number:</strong> {{ $order->quotation_number }}</p>
                    ...
                    <div class="card-body">
                        ...
                        @if($order->items->count())
                            ...
                            <div class="d-flex justify-content-end mb-2">
                                @if($order->status === 'draft' || $order->status === 'sent')
                                    <button type="button" data-quotation='@json($order)' class="btn btn-sm btn-primary edit-items-btn" id="openEditItemsModal" data-bs-toggle="modal" data-bs-target="#editItemsModal">
            ...
            <div class="tab-pane fade" id="tab-communication">
                <div class="card shadow-sm mb-3">
                    <div class="card-body">
                        ...
                        {{-- SEND EMAIL --}}
                        <form action="{{ route('sales-orders.quotations.send-mail', $order->id) }}" method="POST">
                            @csrf
                            <div class="row g-2">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input type="text" name="subject" class="form-control form-control-sm"
                                        value="Request For Quotation {{ $order->invoice_number }}">
                                </div>
            ...
            <div class="tab-pane fade" id="tab-qr">
                <div class="card shadow-sm mb-3 text-center">
                    <div class="card-body">
                        ...
                        <p class="text-muted mt-2">QR encodes: <strong>{{ $order->quotation_number }}</strong></p>
        ...
        <div class="modal fade" id="editItemsModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    ...
                    <form id="editItemsForm" method="POST" action="{{ route('sales-orders.quotations.items.update', $order) }}">
                        @csrf
                        @method('PUT')

                        <div class="modal-header">
                            <h5 class="modal-title my-primary-color">Edit Quotation Items</h5>
        ...
        <div class="d-flex gap-2 mt-3 mb-4">
            @if($order->status === 'draft' || $order->status === 'sent')
                <form action="{{ route('sales-orders.quotations.accept', $order->id) }}" method="POST">
                    @csrf
                    <button type="submit"
                            class="btn btn-success"
            ...
            <a href="{{ route('sales-orders.quotations.print', $order->id) }}" class="btn btn-dark">
                <i class="fa fa-file-pdf"></i> Download PDF
            </a>
            <div class="d-flex gap-2">

                {{-- Convert to Proforma --}}
                @if($order->status === 'accepted' &&  !$order->proforma && !$order->invoice)
                    <button class="btn btn-success" data-bs-target="#generateProformaModal" data-bs-toggle="modal">
                            <i class="fas fa-receipt"></i> Generate PI
                    </button>
                @endif
    ...
    <div class="modal fade" id="updateQuotationModal" tabindex="-1">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <form id="updateQuotationForm" method="POST" action="{{ route('quotations.update', $order) }}">
                    @csrf
                    @method('PUT')

                    <div class="modal-header">
                        <h5 class="modal-title my-primary-color">Update Quotation</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>

                    <div class="modal-body">
                        <input type="hidden" id="update_quotation_id">

                        <div class="row g-3">

                            <div class="col-md-3">
                                <label class="form-label required-field">Quotation Date</label>
                                <input type="date" class="form-control" name="quotation_date" value="{{ $order->invoice_date }}" id="updateQuotationDate">
                            </div>

                            <div class="col-md-3">
                                <label class="form-label required-field">Quotation No</label>
                                <input type="text" class="form-control" id="updateQuotationNumber" value="{{ $order->invoice_number }}" disabled>
                                <input type="hidden" name="quotation_number" value="{{ $order->invoice_number }}" id="updateQuotationNumberHidden">
                            </div>
                            ...
                            <div class="col-md-4 ms-auto">
                                <table class="table table-bordered">
                                    ...
                                    <tr>
                                        <th>Grand Total</th>
                                        <td><input id="update_grand_total" name="grand_total" class="form-control" readonly></td>
                                    </tr>
                                </table>
                            </div>
                            <div class="col-md-12">
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-success">Update Quotation</button>
                    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title">
                        Generate Invoice against this Quotation
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>

                <!-- Modal Body -->
                <form action="{{ route('quotations.convertToInvoice', $order->id) }}"
                    method="POST"
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit"
                                class="btn btn-success"
                                onclick="return confirm('Are you sure you want to convert this quotation to invoice?')">
    ...
    <div class="modal fade" id="generateProformaModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title">
                        Generate PI against this Quotation
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>

                <!-- Modal Body -->
                <form action="{{ route('quotations.convertToProforma', $order->id) }}"
                    method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit"
                                class="btn btn-success"
                                onclick="return confirm('Are you sure you want to convert this Quotation to PI?')">
                            <i class="fas fa-file-invoice"></i> Convert to Proforma Invoice
                        </button>
                    </div>
                </form>
            </div>
...
@push('scripts')
    ...
    @vite('resources/js/pages/sales-order/quotation/show.js')
@endpush